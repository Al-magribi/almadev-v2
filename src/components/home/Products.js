        ...
        <div className='relative z-10 px-6 py-10 md:px-10 md:py-12'>
          <div className='flex flex-col gap-6 md:flex-row md:items-end md:justify-between border-b border-slate-100 pb-8'>
            <div>
              ...
              <p className='text-slate-500 text-sm mt-2 max-w-2xl'>
                Pilihan template, ebook, dan source code yang siap digunakan
                untuk memperkuat portofolio Anda.
              </p>
            </div>
            <Link
              href='/products'
          ...
          <div className='grid grid-cols-1 gap-6 pt-10 md:grid-cols-2 lg:grid-cols-3'>
            {featured.map((product) => {
              ...
              const reviewLabel =
                reviewCount > 0
                  ? `${Number(product.displayRating || 0).toFixed(1)} (${reviewCount.toLocaleString("id-ID")} ulasan)`
                  : "Baru (0 ulasan)";
                ...
                >
                  ...
                  <div className='flex flex-1 flex-col gap-3 p-6'>
                    <div className='flex items-center justify-between text-xs uppercase tracking-widest text-slate-400'>
                      ...
                      <span className='inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-1 text-[11px] font-semibold normal-case tracking-normal text-slate-600'>
